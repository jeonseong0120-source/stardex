...
useEffect(()=>{if(opening!=="box")return;const timer=window.setTimeout(()=>setOpening("tear"),3200);return()=>window.clearTimeout(timer)},[opening]);
  ...
  return <main className="app-shell"><nav className="topbar"><a className="logo" href="#top" onClick={()=>{setView("shop");reset()}}>STARDEX</a><div className="nav-links"><button className={view==="shop"?"active":""} onClick={()=>{setView("shop");reset()}}>PACK SHOP</button><button className={view==="collection"?"active":""} onClick={()=>setView("collection")}>COLLECTION <em>{owned.size}/{cards.length}</em></button></div><div className="balance"><span>◇</span>{coin.toLocaleString()} COIN</div></nav>{view==="collection"?<CollectionExperience cards={cards} owned={owned} showcase={showcase} onShowcaseChange={setShowcase} renderCard={(card,serial)=><CardFace card={card as Card} serial={serial} small/>}/>:opening==="shop"?<Shop coin={coin} buy={buy}/>:<OpeningFlow mode={opening} pack={pack} drag={drag} flipped={flipped} begin={begin} move={move} end={end} flip={flip} reset={reset} owned={owned}/>}<SiteNotice/><footer>STARDEX is a non-commercial fan project. © 2026</footer></main>
}
...
function Shop({coin,buy}:{coin:number;buy:()=>void}){
  return <section id="top" className="lobby-view">
    <div className="lobby-heading">
      <p className="eyebrow">YOUTUBER CREATOR CARD COLLECTION</p>
      <h1>YOUR CREATOR.<br/><i>YOUR CARD.</i></h1>
    </div>
    <div className="box-carousel" aria-label="YouTuber Creator Pack">
      <button className="booster-box booster-box--product is-selected" onClick={buy} disabled={coin<1} aria-label="YouTuber Creator Pack 열기">
