...
export default function Home(){
  ...
  const rarityWeights:Record<PackId,Record<Rarity,number>>={youtube:{COMMON:45,RARE:30,"SUPER RARE":17,MR:5,UR:2,BR:1,"SECRET RARE":0},moe:{COMMON:0,RARE:50,"SUPER RARE":30,MR:12,UR:6,BR:2,"SECRET RARE":0}};
  const packCards=cards.filter(card=>(card.packId??"youtube")===packType);
  const draw=()=>{const available=(Object.entries(rarityWeights[packType]) as [Rarity,number][]).filter(([rarity,weight])=>weight>0&&packCards.some(card=>card.rarity===rarity));const roll=Math.random()*available.reduce((sum,[,weight])=>sum+weight,0);let cursor=0;const rarity=available.find(([ ,weight])=>(cursor+=weight)>roll)?.[0]||packCards[0].rarity;const pool=packCards.filter(card=>card.rarity===rarity);return pool[Math.floor(Math.random()*pool.length)]};
  ...
  const expandShowcase=()=>{if(coin<SHOWCASE_EXPANSION_PRICE)return;setCoin(value=>value-SHOWCASE_EXPANSION_PRICE);setShowcase(slots=>[...slots,...Array(SHOWCASE_EXPANSION_SIZE).fill(null)])};
useEffect(()=>{if(opening!=="box")return;const timer=window.setTimeout(()=>setOpening("tear"),3200);return()=>window.clearTimeout(timer)},[opening]);
  ...
  return <main className="app-shell"><nav className="topbar"><a className="logo" href="#top" onClick={()=>{setView("shop");reset()}}>STARDEX</a><div className="nav-links"><button className={view==="shop"?"active":""} onClick={()=>{setView("shop");reset()}}>팩 샵</button><button className={view==="collection"?"active":""} onClick={()=>setView("collection")}>컬렉션</button></div><div className="topbar-actions"><div className="balance"><span>◇</span>{coin.toLocaleString()} 코인</div><AuthWidget /></div></nav>{view==="collection"?<CollectionExperience cards={cards} owned={owned} showcase={showcase} onShowcaseChange={setShowcase} coin={coin} showcaseExpansionPrice={SHOWCASE_EXPANSION_PRICE} showcaseExpansionSize={SHOWCASE_EXPANSION_SIZE} onExpandShowcase={expandShowcase} renderCard={(card,serial)=><CardFace card={card as Card} serial={serial} small/>}/>:opening==="shop"?<Shop coin={coin} packType={packType} onPackChange={setPackType} packCards={packCards} buy={buy}/>:<OpeningFlow mode={opening} pack={pack} packType={packType} drag={drag} flipped={flipped} begin={begin} move={move} end={end} flip={flip} reset={reset} owned={owned}/>} {view==="shop"&&opening==="shop"&&<SiteNotice/>}<footer>STARDEX는 비상업적 팬 프로젝트입니다. © 2026</footer></main>
}
function SiteNotice(){return <aside className="site-notice" aria-label="STARDEX 안내"><p className="eyebrow">안내</p><h2>STARDEX는 독립 팬 제작 사이트입니다.</h2><p>본 게임 및 사이트는 특정 유튜버·크리에이터와 공식적으로 관련이 없으며, 수익 목적 없이 운영됩니다.</p><div><b>카드 정보 제보</b><span>“XXX님도 추가해주세요.” · “XXX님 내려주세요.” · “XXX님 등급을 변경해주세요.” · “XXX님 카드 정보를 수정해주세요.”</span></div><p>문의 및 수정 제안은 <a href="mailto:jeonseong0120@gmail.com">jeonseong0120@gmail.com</a>으로 부탁드립니다.</p></aside>}
function Shop({coin,buy,packType,onPackChange,packCards}:{coin:number;buy:()=>void;packType:PackId;onPackChange:(id:PackId)=>void;packCards:Card[]}){
  return <section id="top" className="lobby-view">
    <div className="lobby-heading">
      <p className="eyebrow">STARDEX 카드 컬렉션</p>
      <h1>CHOOSE A PACK.</h1>
    </div>
    <div className="box-carousel box-carousel--packs" aria-label="STARDEX 팩 선택">
      ...
      <button className={`booster-box booster-box--product ${packType==="youtube"?"is-selected":""}`} style={{"--side":packType==="youtube"?0:-1,"--order":packType==="youtube"?2:1} as React.CSSProperties} onClick={()=>onPackChange("youtube")} aria-label="크리에이터 팩 선택">
      ...
      <button className={`booster-box booster-box--moe ${packType==="moe"?"is-selected":""}`} style={{"--side":packType==="moe"?0:1,"--order":packType==="moe"?2:1} as React.CSSProperties} onClick={()=>onPackChange("moe")} aria-label="모에몬 컬렉션 팩 선택">
        <img className="moe-box-render" src="/moe-collection-product-v2.png" alt="STARDEX 모에몬 컬렉션 박스와 부스터 팩"/>
      </button>
