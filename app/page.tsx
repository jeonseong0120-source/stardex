...
cards.push(
  ...
  {no:"038",name:"풍자",rarity:"SUPER RARE",flavor:"야식의 여왕",scene:"뜨거운 한 점과 함께 밤을 지배한다",palette:"neon"},
...
function Shop({coin,buy}:{coin:number;buy:()=>void}){const boxes=[{id:"creator",kicker:"SEASON 01",title:"YOUTUBER\nCREATOR PACK",sub:"VOL. 1 · 12 CARDS",live:true},{id:"stream",kicker:"SEASON 02",title:"STREAMER\nARCHIVE",sub:"COMING SOON"},{id:"music",kicker:"SEASON 03",title:"RAPPER\nEDITION",sub:"COMING SOON"}];const[selected,setSelected]=useState(0);useEffect(()=>{const timer=window.setInterval(()=>setSelected(v=>(v+1)%boxes.length),5200);return()=>window.clearInterval(timer)},[]);const selectedBox=boxes[selected];return <section id="top" className="lobby-view"><div className="lobby-heading"><p className="eyebrow">SELECT A BOOSTER BOX</p><h1>CHOOSE<br/><i>YOUR PACK</i></h1></div><div className="box-carousel" aria-label="부스터 박스 선택">{boxes.map((box,index)=>{const position=(index-selected+boxes.length)%boxes.length;const side=position===0?0:position===1?1:-1;return <button key={box.id} className={`booster-box ${box.live?"booster-box--product":""} ${side===0?"is-selected":""}`} style={{"--side":side,"--order":position} as React.CSSProperties} onClick={()=>setSelected(index)} aria-label={`${box.title.replace("\n"," ")} 선택`}>{box.live?<span className="product-box" aria-hidden="true"><span className="product-box__lid"/><span className="product-box__side"/><span className="product-box__front"/></span>:<><span className="box-top"/><span className="box-face"><em>{box.kicker}</em><b>{box.title.split("\n").map(line=><span key={line}>{line}</span>)}</b><small>{box.sub}</small><i>STARDEX</i></span></>}</button>})}</div><div className="carousel-controls"><button onClick={()=>setSelected(v=>(v+boxes.length-1)%boxes.length)} aria-label="이전 박스">←</button><span>{String(selected+1).padStart(2,"0")} / {String(boxes.length).padStart(2,"0")}</span><button onClick={()=>setSelected(v=>(v+1)%boxes.length)} aria-label="다음 박스">→</button></div><div className="selected-box-info"><p>{selectedBox.kicker}</p><b>{selectedBox.live?"YouTuber Creator Pack Vol.1":"다음 컬렉션을 준비 중입니다"}</b><span>{selectedBox.live?"팩 하나에 5장 · 모든 카드 동일 확률":"새 시즌 박스는 추후 공개됩니다"}</span></div><div className="open-area"><button className="open-button" onClick={buy} disabled={!selectedBox.live||coin<1000}>{selectedBox.live?<>OPEN THIS BOX <span>1,000 ◇</span></>:"COMING SOON"}</button><p>{selectedBox.live?"TEST MODE · 모든 카드 동일 확률":"다른 박스를 골라볼 수 있어요"}</p></div></section>}
function OpeningFlow({mode,pack,drag,flipped,begin,move,end,flip,reset,owned}:{mode:Opening;pack:Card[];drag:number;flipped:number[];begin:(e:PointerEvent<HTMLDivElement>)=>void;move:(e:PointerEvent<HTMLDivElement>)=>void;end:()=>void;flip:(i:number)=>void;reset:()=>void;owned:Set<string>}){if(mode==="box")return <section className="box-opening-screen" aria-label="부스터 박스 개봉 중"><p className="eyebrow">UNSEALING BOOSTER BOX</p><div className="opening-assembly"><span className="opening-assembly__shadow"/><div className="opening-assembly__box"><span className="opening-assembly__rear"/><span className="opening-assembly__lid"/><span className="opening-assembly__pack opening-assembly__pack--one"/><span className="opening-assembly__pack opening-assembly__pack--two"/><span className="opening-assembly__pack opening-assembly__pack--three"/><span className="opening-assembly__side"/><span className="opening-assembly__front"/></div></div><p className="box-opening-copy">박스가 열리고, 안쪽에서 팩이 올라오고 있어요.</p></section>;if(mode==="tear")return <section className="opening-screen"><p className="eyebrow">TEAR TO OPEN</p><h2>위로 밀어서<br/><i>팩을 뜯으세요.</i></h2><div className="tear-zone" onPointerDown={begin} onPointerMove={move} onPointerUp={end} onPointerCancel={end}><div className="tear-pack" style={{transform:`translateY(${-drag*.45}px)`,clipPath:`polygon(0 ${drag*.45}%,100% 0,100% 100%,0 100%)`}}><img className="tear-pack__render" src="/booster-pack-v1.png" alt="개봉할 부스터 팩"/></div><div className="tear-line" style={{bottom:`${drag}%`}}><b>✂</b></div></div><p className="drag-hint">{drag?`${Math.round(drag)}%`:`SWIPE UP`}</p></section>;if(mode==="deal")return <section className="opening-screen deal-screen"><p className="eyebrow">TAP EACH CARD TO REVEAL</p><h2>무엇이 나왔을까.</h2><div className="reveal-fan">{pack.map((card,index)=><div key={`${card.no}-${index}`} onClick={()=>flip(index)}><CardBack index={index} flipped={flipped.includes(index)}><CardFace card={card} serial={pullsSerial(index)}/></CardBack></div>)}</div><p className="drag-hint">{flipped.length} / 5 REVEALED</p></section>;const fresh=pack.filter(c=>!owned.has(c.no)).length;return <section className="result-screen"><p className="eyebrow">PACK RESULTS</p><h2>COLLECTION<br/><i>UPDATED</i></h2><div className="result-grid">{pack.map((card,index)=><div className="result-item" key={`${card.no}-${index}`}><CardFace card={card} serial={pullsSerial(index)} small/><b className={owned.has(card.no)?"duplicate":"new"}>{owned.has(card.no)?"DUPLICATE +1":"NEW"}</b></div>)}</div><p className="summary"><b>{fresh} NEW CARD{fresh===1?"":"S"}</b> · 중복 카드는 추후 Dust로 분해할 수 있어요.</p><button className="open-button" onClick={reset}>BACK TO SHOP</button></section>}
const pullsSerial=(index:number)=>Math.floor(Date.now()/1000)%100000+index;
function Collection({owned}:{owned:Set<string>}){return <section className="collection-view"><header><div><p className="eyebrow">MY ARCHIVE</p><h1>YOU TUBE<br/><i>COLLECTION</i></h1></div><p className="progress"><b>{owned.size}</b> / {cards.length}<br/><span>CARDS DISCOVERED</span></p></header><div className="collection-grid">{cards.map((card,index)=>owned.has(card.no)?<button key={card.no} className="collection-card"><CardFace card={card} serial={index+1} small/></button>:<div className="locked-card" key={card.no}><span>?</span><p>NO. {card.no}</p></div>)}</div></section>}
