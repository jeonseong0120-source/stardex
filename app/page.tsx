...
useEffect(()=>{if(opening!=="box")return;const timer=window.setTimeout(()=>setOpening("tear"),3200);return()=>window.clearTimeout(timer)},[opening]);
  ...
  const flip=(index:number)=>{if(revealing||flipped.includes(index))return;const next=[...flipped,index],rarity=pack[index].rarity,effect=rarity==="SUPER RARE"?"pull-sr":rarity==="MR"?"pull-mr":rarity==="UR"?"pull-ur":rarity==="BR"?"pull-br":"",isBr=rarity==="BR",duration=isBr?1450:900;setRevealing(true);if(effect)document.body.classList.add(effect);if(isBr)setTimeout(()=>setFlipped(next),520);else setFlipped(next);setTimeout(()=>{if(effect)document.body.classList.remove(effect);setRevealing(false)},duration);if(next.length===5){setPulls(v=>[...v,...pack]);setTimeout(()=>setOpening("result"),duration+120)}};
...
function Shop({coin,buy}:{coin:number;buy:()=>void}){const boxes=[{id:"creator",kicker:"SEASON 01",title:"YOUTUBER\nCREATOR PACK",sub:"VOL. 1 · 12 CARDS",live:true},{id:"stream",kicker:"SEASON 02",title:"?",sub:"입고 준비중입니다."},{id:"music",kicker:"SEASON 03",title:"?",sub:"입고 준비중입니다."}];const[selected,setSelected]=useState(0);useEffect(()=>{const timer=window.setInterval(()=>setSelected(v=>(v+1)%boxes.length),5200);return()=>window.clearInterval(timer)},[]);const selectedBox=boxes[selected];return <section id="top" className="lobby-view"><div className="lobby-heading"><p className="eyebrow">SELECT A BOOSTER BOX</p><h1>CHOOSE<br/><i>YOUR PACK</i></h1></div><div className="box-carousel" aria-label="부스터 박스 선택">{boxes.map((box,index)=>{const position=(index-selected+boxes.length)%boxes.length;const side=position===0?0:position===1?1:-1;return <button key={box.id} className={`booster-box ${box.live?"booster-box--product":""} ${side===0?"is-selected":""}`} style={{"--side":side,"--order":position} as React.CSSProperties} onClick={()=>setSelected(index)} aria-label={`${box.title.replace("\n"," ")} 선택`}>{box.live?<span className="product-box" aria-hidden="true"><span className="product-box__lid"/><span className="product-box__side"/><span className="product-box__front"/></span>:<span aria-hidden="true" style={{display:"grid",placeItems:"center",width:"100%",height:"100%",color:"#d8d2eb",font:"clamp(5rem,12vw,9rem)/1 Georgia,serif",filter:"drop-shadow(0 0 20px #b28cff66)"}}>?</span>}</button>})}</div><div className="carousel-controls"><button onClick={()=>setSelected(v=>(v+boxes.length-1)%boxes.length)} aria-label="이전 박스">←</button><span>{String(selected+1).padStart(2,"0")} / {String(boxes.length).padStart(2,"0")}</span><button onClick={()=>setSelected(v=>(v+1)%boxes.length)} aria-label="다음 박스">→</button></div><div className="selected-box-info"><p>{selectedBox.kicker}</p><b>{selectedBox.live?"YouTuber Creator Pack Vol.1":"입고 준비중입니다."}</b><span>{selectedBox.live?"팩 하나에 5장 · 모든 카드 동일 확률":"다음 카드박스를 준비하고 있어요"}</span></div><div className="open-area"><button className="open-button" onClick={buy} disabled={!selectedBox.live||coin<1}>{selectedBox.live?<>OPEN THIS BOX <span>1 ◇</span></>:"입고 준비중입니다."}</button><p>{selectedBox.live?"TEST MODE · 모든 카드 동일 확률":"다른 박스를 골라볼 수 있어요"}</p></div></section>}
